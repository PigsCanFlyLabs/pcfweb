...
Several classes, because independent features each added columns needing
...
``PostgresColumnDefaultDDLTest``       the stock column (from the product
                                       ...
                                       ``(sql, params)`` pairs.
``RollingDeployOldCodeProductInsertTest``
                                       the per-format identifier columns,
                                       which are NULLable rather than
                                       defaulted -- the other safe answer.
``ProductFormatIdentifierMigrationTest``
                                       the isbn -> print_isbn backfill.
