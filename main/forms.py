...
class MailingListSignupForm(forms.Form):
    ...
    # The "send me everything" checkbox. Ticked subscribes them to the All
    # list *instead of* the topic (All already receives every public mailing,
    # so one row covers both); unticked means only the topic, which is the
    # promise the subscribe page makes. See mailing.subscribe.
