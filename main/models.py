...
def absolute_site_url(path: str, request=None) -> str:
    ...
    # SITE_BASE_URL, not a mailing-list-specific setting: the site has one
    # absolute base, and the emailed download links already use this one.
    from main.digital import site_base_url
...
class MailingListMessage(models.Model):
    ...
    def recipients(self) -> "models.QuerySet":
        ...
        if interests:
            ...
            if any(interest.visible for interest in interests):
                ...
                #
                # "Public" is the limit, and it is what makes the internal test
                # list usable: All means every list somebody could have
                # subscribed to, and a hidden list is not one of those. Without
                # that, a mailing aimed at the test list would go to the whole
                # All list, which is the opposite of what it is for.
